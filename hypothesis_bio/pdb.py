...
from hypothesis.strategies import composite, dates, integers, text, characters, floats
...
def generate_token(draw, min_size=1, max_size=None):
    ...
    """
    token = draw(text(alphabet=characters(min_codepoint=33, max_codepoint=126), min_size=min_size, max_size=max_size))
    return token + ": "
...
def generate_lstring(draw, min_size=1, max_size=None):
    ...
    """
    return draw(text(alphabet=characters(min_codepoint=32, max_codepoint=126), min_size=min_size, max_size=max_size))
...
@composite
def generate_specification(draw, min_token_size=1, max_token_size=None, min_value_size=1, max_value_size=None):
...
def generate_header(draw):
    ...
    return "HEADER" + " " * 3 + classification.ljust(40, ' ') + depDate + " " * 3 + idCode
